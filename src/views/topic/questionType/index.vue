    ...
    <div class="search-form">
      ...
      >
        ...
        <el-button
          ...
          class="add"
          plain
          @click="openDialog({ title: '新增题型' });isAdd = true;
            subData = {};"
          >新增题型</el-button
        >
      </el-form>
...
export default {
  ...
  methods: {
    async loadData(isSearch) {
      ...
      const res = await this.$api.questionType.list(
    ...
    toEdit(row) {
      this.isAdd = false;
      this.subData = row;
      this.openDialog({ title: "编辑题型" });
    },
    // 弹出框确认回调
    async handleSubmit(params) {
      this.$api.questionType.add(params).then((res) => {
        if (res.code == 0) {
    ...
    async handleEdit(params) {
      this.$api.questionType.edit(params).then((res) => {
        if (res.code == 0) {
          this.$message.success(res.message);
          this.loadData();
          this.closeDialog();
    ...
    handleDelete(row) {
      this.$confirm("确认删除该题型吗", "确认信息", {
        ...
        .then(async () => {
          // 删除确认回调
          this.$api.questionType.delete(row).then((res) => {
            if (res.code == 0) {
              this.$message.success(res.message);
              this.loadData();
              this.closeDialog();
